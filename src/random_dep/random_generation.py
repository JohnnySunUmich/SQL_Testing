...
def fill_primary_keys(fake: Faker, tables, table_schema, num_row, output, cstr):
    ...
    unique_perc = [u / 10 for u in unique_perc]
    if len(pkeys) == 1:
        unique_perc = [1]
    dimension = []
...
def enforce_duplicate(table_schema, num_row, output):
    ...
    # dup_table[1]: random_dep.uniform(0.0, dup_table[1]) to determine number of rows to duplicate to
...
def type_functions(fake: Faker, col):
    ...
    if "int" in col["Type"]:
        ...
        gen_unique = lambda k: [truncate(fake.unique.name()) for _ in range(k)]

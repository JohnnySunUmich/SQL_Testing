...
from util import dif_table, readable_db
from psql_util import run_psql
from mysql_util import run_mysql
from constrained_generation import Constrained_Random_Table_Generation
import time
import logging
import timeit
...
class Tester():
    ...
    def run_query(self, query, testing_database):
        ...
        elif self.engin == 'mysql':
            return run_mysql(query, testing_database, self.schema, self.conf['mysql'])

    def generate_databse(self, is_random=True):
        """
        generate a database that follows the schema
        currently only support random_dep generation
        """
        if is_random:
            seed = time.time()
    ...
    def random_kill(self, query1, query2,
        ...
        while True:
            ...
            if nonempty:
                if len(outputs[0]) == 0 and len(outputs[0]) == 0:
                    continue
            found_nonempty = True
    ...
    def random_testing(self, reference: str, testing: Union[str, list],
        ...
        if isinstance(testing, list):
            ...
            for query in testing:
                res, extra_info = self.random_testing(reference, query)
                results += res
                extra_infos += extra_info
            return results, extra_infos
        else:
        ...
        if isinstance(testing, list):
            ...
            for database in self.database_buffer:
                ...
                if dif_table(outputs[0], outputs[1], order):
                    ...
                    logging_output += f"outputs are\n{outputs[0]}\n{outputs[1]}"
                    return [True], [{'outputs': logging_output}]
            return [False], [{}]